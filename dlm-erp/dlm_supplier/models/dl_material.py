...
class DlMaterial(models.Model):
    ...
    supplier_id = fields.Many2one(
        ...
        domain=[('partner_role', '=', 'supplier'), ('active', '=', True)])

    @api.constrains('supplier_id')
    def _check_supplier_role(self):
        for rec in self:
            if rec.supplier_id and rec.supplier_id.partner_role != 'supplier':
                raise ValidationError(
                    'NCC mặc định phải là một Nhà cung cấp (partner_role = supplier).'
                )
