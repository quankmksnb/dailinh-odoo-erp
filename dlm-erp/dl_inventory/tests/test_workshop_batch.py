...
class TestWorkshopBatch(DlInventoryCase):
    ...
    def test_dai_thay_dong_vua_go_khi_phieu_chua_luu(self):
        """TC-INT-TestWorkshopBatch-001: Dùng `Form` vì đây là ca chỉ xảy ra qua giao thức
        onchange của client.

        Vấp thật khi test tay: gõ 2 dòng vào bảng "Xưởng nộp về", dải đỏ vẫn nói "Phiếu
        chưa có dòng nào" và nút Xác nhận biến mất thì người dùng nhìn thấy dòng của
        mình trên màn mà không có đường nào đi tiếp.

        Nguyên nhân: `move_ids`, `dlm_fg_move_ids`, `dlm_material_move_ids` cùng một
        inverse nhưng là BA ô độc lập với client — gõ vào ô này thì ô kia rỗng cho tới
        lúc Lưu. Mọi test `create()` đều bỏ lọt vì `create()` ghi thẳng xuống DB, nơi cả
        ba ô trỏ cùng một tập.
        """
        picking_form = Form(
        ...
        # `_compute_state` không bao giờ chạy thì `state` = False, không phải
    ...
    def test_tron_mot_me_bon_con_so_cuoi_deu_dung(self):
        """TC-INT-TestWorkshopBatch-002: Cấp 100 thì bổ sung 10 thì làm ra 10 bàn, dùng 110
        thép, cân 43kg phế.

        Đây là kịch bản nghiệm thu của K16. Bốn con số cuối phải khớp thực tế: không
        thép ma ở Xưởng, không tồn âm, phế liệu có chỗ nằm.
        """
    ...
    def test_khong_co_ton_am_o_vi_tri_noi_bo(self):
        """TC-INT-TestWorkshopBatch-003: Lá chắn tổng: một quant âm ở khu nội bộ là dấu
        hiệu luồng đã sai.

        Chỉ soi `usage='internal'`. Vị trí ảo Sản xuất âm là đúng và bắt buộc — nó là vế
        đối ứng của việc hàng được sinh ra: 10 cái bàn vào Kho thành phẩm thì Sản xuất
        phải nợ 10 cái. Bắt cả vị trí ảo là bắt sai chỗ, và sẽ đẩy người sửa đi bịt đúng
        thứ đang chạy đúng.
        """
    ...
    def test_vi_tri_suy_tu_vai_tro_dong_va_mat_hang(self):
        """TC-INT-TestWorkshopBatch-004: Ba loại dòng, ba cặp vị trí — người dùng không có
        ô nào để chọn sai.
        """
    ...
    def test_khai_vat_tu_vuot_ton_o_xuong_bi_chan(self):
        """TC-INT-TestWorkshopBatch-006: Khác chính sách SM-03: ở đây "thiếu" không phải
        phiếu treo mà là tồn âm — Odoo không chặn tồn âm nội bộ nên nó hỏng im lặng.
        """
    ...
    def test_hang_khong_vao_ton_phai_gan_don(self):
        """TC-INT-TestWorkshopBatch-007: Hạng A không có dòng tồn nào để tra ngược thì đơn
        là danh tính duy nhất.
        """
    ...
    def test_cap_bo_sung_phai_co_ly_do(self):
        """TC-INT-TestWorkshopBatch-008: ô tick một mình không nói được gì — lý do mới là
        dữ liệu.
        """
        self._stock_at(self.loc_kho, self.material, 10.0)
        picking = self.env["stock.picking"].create({
    ...
    def test_khai_100_nhan_98_thi_ton_tang_98_va_khong_phieu_bu(self):
        """TC-INT-TestWorkshopBatch-009: Thủ kho là nút chặn cuối. Xưởng khai 100, đếm thực
        98 thì sổ ghi 98.

        Và không sinh phiếu bù: 2 cái còn nằm ở xưởng, mẻ sau khai tiếp — chứ không phải
        một phiếu rỗng treo vĩnh viễn trong hàng đợi thủ kho.
        """
    ...
    def test_ky_thuat_lap_va_xac_nhan_duoc_phieu_me(self):
        """TC-INT-TestWorkshopBatch-011: Chạy dưới quyền thật, không sudo — đây là ca mà
        mọi test khác trong file này bỏ lọt vì chúng chạy bằng superuser.

        Lỗ đã vấp thật: `action_confirm` của Odoo đụng `stock.warehouse.orderpoint` (quy
        tắc tồn kho tối thiểu), mà bên Kỹ thuật không có ACL đọc model đó thì họ lưu
        được phiếu nhưng không xác nhận nổi, và câu lỗi nói về một model chẳng liên quan
        gì tới việc họ đang làm.
        """
    ...
    def test_ky_thuat_khong_ky_nhan_thay_thu_kho(self):
        """TC-INT-TestWorkshopBatch-012: Bên nhận của phiếu [8] là Thủ kho. Xưởng ký cả hai
        đầu thì lần truy cứu sau không phân biệt nổi ai giao với ai nhận.
        """
        tech = self.env["res.users"].create({
