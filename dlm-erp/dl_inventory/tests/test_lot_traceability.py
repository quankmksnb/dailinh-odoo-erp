# -*- coding: utf-8 -*-
"""K4 — Truy vết lô + hai màn hình đầu tiên của phân hệ Kho.

Thiết kế: docs/Thiet_ke_phan_he_kho.md §5.5, §11.2, §11.9.

Chuỗi truy vết phải đi được tới tận NCC:
    khách báo hỏng → đơn → BOM → lô vật tư → phiếu nhận → NCC + ngày nhập
Bộ test này khoá chặng cuối (lô → phiếu nhận → NCC).
"""
...
class TestLotTraceability(DlInventoryCase):
    ...
    def test_nguon_goc_khong_bi_ghi_de_boi_luan_chuyen_sau(self):
        """§5.5 — Lô sinh ra từ lần nhập ĐẦU TIÊN.

        Nếu mỗi lần hàng đi qua một phiếu lại ghi đè, truy vết sẽ trỏ về phiếu
        chuyển kho nội bộ gần nhất thay vì về NCC — mất sạch giá trị.
        """
        picking = self._receive(self._make_receipt())
        lot = picking.move_line_ids.lot_id
    ...
    def test_man_ton_kho_doc_duoc_ncc_va_ngay_nhap(self):
        """§11.2 — Màn Tồn kho hiện NCC + ngày nhập mà không phải mở từng lô."""
        picking = self._receive(self._make_receipt())
    ...
    def test_man_ton_kho_khong_lo_gia(self):
        ...
        Kiểm ngay trên VIEW (không chỉ ACL): cột giá lọt vào view là rò ngay,
        dù ACL có đúng.
        """
    ...
    def test_menu_kho_co_muc_con_de_hien_tren_rail(self):
        ...
        Bẫy đã ghi trong CLAUDE.md: rail lọc bằng _hasActionableMenu ⇒ menu lá
        rỗng action biến mất khỏi thanh điều hướng mà không lỗi nào nổ.
        """
    ...
    def test_thu_kho_vao_duoc_app_dlm(self):
        ...
        Bẫy đã ghi trong CLAUDE.md: Odoo bắt phải pass groups **CẢ TỔ TIÊN**.
        Cấp quyền cho menu con mà quên menu gốc ⇒ user đăng nhập vào KHÔNG thấy
        app đâu cả, và không có lỗi nào nổ — chỉ là màn hình trống.
        """
