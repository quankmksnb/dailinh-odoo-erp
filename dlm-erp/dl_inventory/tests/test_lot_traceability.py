...
class TestLotTraceability(DlInventoryCase):
    ...
    def test_lo_ghi_nguon_goc_khi_nhan_hang(self):
        """TC-INT-TestLotTraceability-001: §5.5: nhận hàng xong thì lô phải biết mình của
        ai, về ngày nào.

        Không có 3 field này, số lô chỉ là chuỗi ký tự vô nghĩa. Khách báo nứt mối hàn
        vẫn không tra được thép mua của ai.
        """
    ...
    def test_nguon_goc_khong_bi_ghi_de_boi_luan_chuyen_sau(self):
        """TC-INT-TestLotTraceability-002: §5.5: nguồn gốc lô phải giữ thông tin của lần
        nhập đầu tiên.

        Nếu mỗi lần hàng đi qua một phiếu lại ghi đè, truy vết sẽ trỏ về phiếu chuyển
        kho nội bộ gần nhất thay vì về NCC, mất sạch giá trị truy vết.
        """
        picking = self._receive(self._make_receipt())
        lot = picking.move_line_ids.lot_id
    ...
    def test_lo_mo_duoc_phieu_nhan_goc(self):
        """TC-INT-TestLotTraceability-003: §11.9: nút trên form Lô hàng dẫn thẳng về phiếu
        nhận.
        """
        picking = self._receive(self._make_receipt())
        lot = picking.move_line_ids.lot_id
    ...
    def test_man_ton_kho_doc_duoc_ncc_va_ngay_nhap(self):
        """TC-INT-TestLotTraceability-004: §11.2: màn Tồn kho hiện NCC và ngày nhập mà
        không phải mở từng lô.
        """
        picking = self._receive(self._make_receipt())
    ...
    def test_man_ton_kho_khong_lo_gia(self):
        """TC-INT-TestLotTraceability-005: §8.3: Thủ kho mở màn Tồn kho không được thấy
        tiền.

        Kiểm ngay trên view (không chỉ ACL): cột giá lọt vào view là lộ ngay, dù ACL có
        đúng.
        """
    ...
    def test_menu_kho_co_muc_con_de_hien_tren_rail(self):
        """TC-INT-TestLotTraceability-006: §11.11: menu lá phải có action, nếu không rail
        sẽ nuốt mất mục.

        Bẫy đã ghi trong CLAUDE.md: rail lọc bằng _hasActionableMenu, nên menu lá rỗng
        action sẽ biến mất khỏi thanh điều hướng mà không lỗi nào nổ.
        """
    ...
    def test_thu_kho_vao_duoc_app_dlm(self):
        """TC-INT-TestLotTraceability-007: Thủ kho phải thấy menu gốc "DLM-ERP", không chỉ
        menu "Kho".

        Bẫy đã ghi trong CLAUDE.md: Odoo bắt phải pass groups của cả tổ tiên. Cấp quyền
        cho menu con mà quên menu gốc thì user đăng nhập vào không thấy app đâu cả, và
        không có lỗi nào nổ, chỉ là màn hình trống.
        """
        ...
        for xml_id in ("dl_base.menu_dl_root", "dl_base.menu_dl_inventory"):
            ...
            self.assertIn(
                ...
                "Thủ kho không thấy được %s thì đăng nhập vào là màn trống."
                % xml_id)

    def test_thu_kho_mo_duoc_hai_man(self):
        """TC-INT-TestLotTraceability-008: K4: vai trò Thủ kho thực sự đọc được dữ liệu của
        hai màn mới.
        """
