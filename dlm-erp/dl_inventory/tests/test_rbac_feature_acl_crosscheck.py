...
Trước đây CHƯA có test nào đối chiếu 2 nguồn này — một feature khai model_id
sai/mồ côi (model không còn tồn tại, hoặc Thủ kho không thực sự có quyền đọc
model đó) sẽ khiến màn Phân quyền hiển thị sai mà không có test nào bắt được.
"""
from .common import DlInventoryCase

# code -> có model_id hay cố ý bỏ trống (kiểm kê/phế liệu không có model
...
class TestRbacFeatureAclCrosscheck(DlInventoryCase):
    ...
    def test_toan_bo_6_feature_kho_deu_ton_tai(self):
        """TC-INT-TestRbacFeatureAclCrosscheck-001 — đủ 6 bản ghi K1, không
        thiếu/không trùng code (code_uniq)."""
        features = self._features()
        self.assertEqual(
            sorted(features.mapped("code")), sorted(_EXPECTED_FEATURES))

    def test_model_id_dung_hoac_co_y_bo_trong(self):
        """TC-INT-TestRbacFeatureAclCrosscheck-002 — model_id của mỗi feature
        khớp đúng model kỳ vọng, hoặc cố ý trống (kiểm kê/phế liệu)."""
    ...
    def test_thu_kho_co_quyen_doc_moi_model_co_model_id(self):
        """TC-INT-TestRbacFeatureAclCrosscheck-003 — bất biến chính: mọi
        feature K1 có model_id đều phải có ít nhất 1 dòng ir.model.access
        cấp quyền ĐỌC cho dl_group_warehouse trên đúng model đó. Feature khai
        model_id mà Thủ kho không đọc được là feature mồ côi — màn Phân
        quyền sẽ hiện mục vô dụng."""
    ...
    def test_khong_co_feature_stock_nao_ngoai_6_cai_da_biet(self):
        """TC-INT-TestRbacFeatureAclCrosscheck-004 — không phát sinh thêm
        feature code tiền tố stock_ nào ngoài 6 cái đã khai (đổi code = sinh
        bản ghi mới, feature cũ mồ côi — theo đúng cảnh báo trong comment
        rbac_features.xml)."""
