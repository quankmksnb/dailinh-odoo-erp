...
dl_inventory). File này chỉ test `_dlm_is_fully_delivered`, hàm duy nhất
...
Các method khác trong dl_inventory/models/dl_sale_order.py
(_dlm_needed_qty/_dlm_delivered_qty/_dlm_planned_qty/_dlm_remaining_qty,
action_dlm_create_delivery...) đều đụng self.env/recordset thật nên thuộc
L2, xem sheet TestDeliveryLink trong Report 5.2_IntegrationTests_L2.xlsx.
...
class TestDlmIsFullyDelivered(unittest.TestCase):
    ...
    def test_delivered_more_than_needed_is_fully_delivered(self):
        """TC-UNIT-DlSaleOrder-016: giao dư (ví dụ do làm tròn ở tầng move) vẫn
        tính là đã giao đủ."""
        thep = _product()
        needed = {thep: 100.0}
    ...
    def test_product_missing_from_delivered_dict_counts_as_zero(self):
        ...
        trong `delivered` (chưa có move nào done) thì coi như đã giao 0."""
        thep = _product()
        needed = {thep: 50.0}
        delivered = {}
        self.assertFalse(
    ...
    def test_empty_needed_is_vacuously_fully_delivered(self):
        """TC-UNIT-DlSaleOrder-019: không có sản phẩm nào cần giao qua kho thì
        vòng lặp rỗng, trả True. Trạng thái này không phát sinh trong thực tế
        vì _compute_dlm_delivery_state đã chặn ở nhánh "nothing" trước khi gọi
        tới hàm này, nhưng vẫn cần đúng nếu gọi trực tiếp."""
        self.assertTrue(
            DlSaleOrder._dlm_is_fully_delivered(_order(), {}, {}))

    def test_multiple_products_one_short_blocks_all(self):
        """TC-UNIT-DlSaleOrder-020: nhiều sản phẩm, chỉ 1 sản phẩm giao thiếu
        thì toàn đơn vẫn chưa "done" (không tính trung bình)."""
        thep = _product()
        ton = _product()
    ...
    def test_falls_back_to_default_rounding_when_uom_rounding_falsy(self):
        """TC-UNIT-DlSaleOrder-022: `rounding` của UoM bằng 0/False thì hàm
        dùng mặc định 0.01 (`product.uom_id.rounding or 0.01`), tránh chia
        cho 0 và tránh coi mọi chênh lệch là hợp lệ."""
