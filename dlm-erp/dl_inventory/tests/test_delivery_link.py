...
Ba bất biến đắt nhất nếu sai:
  ...
  1. Đơn chưa chốt không được sinh phiếu giao, vì phiếu giao sẽ giữ chỗ hàng
     cho một cam kết chưa tồn tại, khiến hàng "biến mất" khỏi tồn khả dụng
     của đơn thật.
  2. SP dùng chung (Hạng A, ``consu``) không lên phiếu giao vì nó không có
     tồn, dòng sinh ra sẽ treo vĩnh viễn ở trạng thái chờ hàng.
  3. Đơn đã có phiếu giao không đưa về nháp được, vì sửa lại đơn đã giao sẽ
     làm lệch chứng từ kho với chứng từ bán.
"""
...
class TestDeliveryLink(DlInventoryCase):
    ...
    def test_tao_phieu_giao_tu_don_da_xac_nhan(self):
        """Đơn confirmed thì bấm [Tạo phiếu giao] phải ra đúng một phiếu, đúng số lượng."""
    ...
    def test_giao_du_thi_don_chuyen_sang_da_giao_du(self):
        """Tiêu chí verify K6: giao xong thì dlm_delivery_state phải là 'done'."""
        order = self._make_order(qty=10.0)
    ...
    def test_giao_mot_phan_thi_don_o_trang_thai_giao_mot_phan(self):
        """Giao 6/10 phải là 'partial', không được báo 'done' khi còn nợ khách 4."""
        order = self._make_order(qty=10.0)
