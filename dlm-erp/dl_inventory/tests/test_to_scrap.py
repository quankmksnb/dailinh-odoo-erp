...
Hai mốc này bắt buộc đi cùng nhau, và bộ test canh đúng lý do đó: K11 bịt đường vòng qua
khu cha `DL/NHAN` — mà đó đang là lối ra duy nhất của hàng kẹt ở khu Chờ trả NCC. Bịt
trước mà không mở lối sau thì có một khoảng thời gian 8 cây thép gỉ không ra được bằng
đường nào: cấm kiểm kê tay (RS-07), cấm làm nguồn phiếu chuyển kho (§4.1.1), và phiếu
trả thì đã huỷ.

Cái sai đắt nếu K11 hỏng, xếp theo mức nặng:

1. phiếu giao hàng lấy nguồn "Chờ kiểm hàng" thì giao thẳng hàng chưa kiểm cho khách —
nặng hơn hẳn hai cửa Chuyển kho/Kiểm kê cộng lại; 2. khu Phế liệu nhận cả thép nguyên
cây thì bán ve chai thép mới, và Lệnh sản xuất (B2) lại rút chính nó ra làm hàng; 3.
chọn khu cha `DL/NHAN` làm nguồn thì rút thẳng từ Chờ kiểm / Chờ trả.
"""
...
class TestToScrap(DlInventoryCase):
    ...
    def _reject_flow(self, nhan=10.0, loai=8.0):
        """Nhận hàng thì kiểm thì loại `loai` cây thì phiếu trả nháp + hàng ở Chờ trả.
        """
        ...
        # button_validate không dời hàng thì không có hàng nào tới khu Chờ trả.
        qc.move_ids[:1].write({
            "quantity": nhan - loai,
            "picked": True,
    ...
    def test_k11_khu_nhap_cha_cam_chon_tay(self):
        """TC-INT-TestToScrap-001: Cửa 1: khu CHA `DL/NHAN` phải mang cờ, không chỉ hai khu
        con.

        K10 biến thiếu sót này thành lỗ thật: sau khi Kho vật tư dời xuống Xưởng, con
        của DL/NHAN chỉ còn hai khu quá cảnh thì chọn khu cha là rút thẳng từ Chờ kiểm /
        Chờ trả, đi vòng qua thằng cha nhưng cùng một lỗ.
        """
    ...
    def test_k15_khu_gom_nhom_kho_san_xuat_cam_chon_tay(self):
        """TC-INT-TestToScrap-002: Cửa thứ tư, sinh ra cùng lúc với khu mới: `DL/KHOSX` là
        container.

        Cùng một lỗ với cửa 1 (`DL/NHAN`): chọn khu cha làm nguồn là rút được từ mọi ô
        con — ở đây là Kho nguyên vật liệu và Phế liệu chờ bán. Viết test ngay lúc tạo
        khu, không đợi nó thành sự cố như K10 thì K11.
        """
    ...
    def test_k11_giao_hang_nguon_cho_kiem_bi_chan(self):
        """TC-INT-TestToScrap-004: Cửa 3 — nặng nhất: guard cũ chỉ áp cho `transfer` nên
        phiếu GIAO hàng lấy nguồn Chờ kiểm lọt sạch thì giao hàng chưa kiểm cho khách.
        """
        picking = self._picking(
    ...
    def test_k11_domain_form_giao_hang_co_loc_khu_qua_canh(self):
        """TC-INT-TestToScrap-005: Canh trên chính `arch` của view: guard server đúng mà
        dropdown vẫn mời chọn thì người dùng chọn xong mới ăn lỗi.
        """
    ...
    def test_k11_phe_lieu_thi_vao_duoc(self):
        """TC-INT-TestToScrap-007: Chiều ngược: luật chỉ trả 'cấm' cho mọi thứ thì test
        trên vẫn xanh mà khu Phế liệu thành khu chết.
        """
        picking = self._picking(
            self.warehouse.int_type_id, self.loc_xuong, self.loc_xuong_pl,
            product=self.scrap)
    ...
    def test_k11_luat_khu_phe_lieu_khong_doc_product_kind(self):
        """TC-INT-TestToScrap-008: SCRAP-STEEL mang `product_kind='material'` y hệt thép
        thật — luật phải đọc cờ, không đọc loại hàng. Đây là luật đầu tiên như thế.
        """
    ...
    def test_k12_hoa_phe_lieu_doi_mat_hang(self):
        """TC-INT-TestToScrap-010: Bất biến của phiếu [9]: hàng gốc rời sổ và phế liệu vào
        khu chờ bán — hai nửa, một chứng từ. Thiếu nửa giảm thì 8 cây thép gỉ vẫn nằm
        trong sổ như hàng dùng được.
        """
    ...
    def test_k12_thieu_ly_do_thi_chan(self):
        """TC-INT-TestToScrap-012: Bút toán làm hàng biến mất khỏi sổ — không lý do thì sáu
        tháng sau không ai giải thích nổi vì sao 8 cây thép bốc hơi.
        """
        ret = self._reject_flow()
        ret.action_cancel()
    ...
    def test_k12_giu_dung_lo_dang_bo(self):
        """TC-INT-TestToScrap-014: Giữ chỗ theo đúng lô đang bỏ, không để chiến lược lấy
        hàng chọn hộ.
        """
        ret = self._reject_flow()
        ret.action_cancel()
    ...
    def test_k12_khong_hoa_phe_lieu_chinh_phe_lieu(self):
        """TC-INT-TestToScrap-015: Phế liệu đã là phế liệu — hoá lần nữa là vòng lặp vô
        nghĩa và làm biến mất chính thứ đang chờ bán.
        """
    ...
    def test_k12_khong_hoa_phe_lieu_thang_tu_khu_qua_canh(self):
        """TC-INT-TestToScrap-016: Màn Tồn kho không được là cửa sau vào khu Chờ trả: mất
        dấu vết lô hàng đi từ phiếu trả nào ra.
        """
        ret = self._reject_flow()
        ret.action_cancel()
    ...
    def test_k12_vat_tu_hong_trong_kho_hoa_duoc(self):
        """TC-INT-TestToScrap-017: Lối vào thứ hai (§6.5): vật tư gỉ trong Kho vật tư,
        không đòi NCC.
        """
