...
Không gắn 1 endpoint/controller cụ thể (khác các test RBAC record-rule như
LKT-12 trong test_bom_btp_linkage.py) — kiểm field-level `groups=` trên
dl.bom.line (price_snapshot, recovery_value, subtotal) và dl.bom
(total_operation_cost_est), cơ chế Odoo lõi tại
...
class TestCostFieldVisibilitySecurity(TransactionCase):
    ...
    # ------------------------------------------------------------------
    # dl.bom — total_operation_cost_est (ước tính chi phí công đoạn/đv)
    # ------------------------------------------------------------------
