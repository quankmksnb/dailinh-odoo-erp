...
class TestCronCleanupRfqProvisional(TransactionCase):
    ...
    def test_orphan_provisional_bom_older_than_threshold_is_cleaned(self):
        ...
        còn được dòng RFQ nào tham chiếu, bỏ dở quá 7 ngày → bị dọn."""
    ...
    def test_recent_provisional_bom_within_threshold_kept(self):
        ...
        qua ngưỡng 7 ngày) → KHÔNG bị dọn (biên dưới)."""
        product = self.env["product.product"].create({
    ...
    def test_non_provisional_bom_never_touched(self):
        """TC-SCH-CRON-RFQPROV-003 — BOM KHÔNG phải tạm RFQ (is_rfq_provisional
        =False), dù cũ đến đâu, cũng không nằm trong phạm vi cron này."""
        product = self.env["product.product"].create({
    ...
    def test_no_provisional_records_runs_without_error(self):
        """TC-SCH-CRON-RFQPROV-004 — không có bản ghi tạm nào → cron chạy
        không lỗi."""
        result = self.env[
