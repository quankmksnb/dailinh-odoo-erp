# -*- coding: utf-8 -*-
"""L2 Integration test (TransactionCase, DB thật) cho res.users dlm_* —
...
class TestResUsers(TransactionCase):
    ...
    def test_create_user_duplicate_login_raises(self):
        """TC-INT-ResUsers-003 — kể cả user đã bị khoá (active=False) vẫn
        tính là trùng (search dùng active_test=False)."""
        Users = self.Users.with_user(self.admin_user)
    ...
    def test_create_user_filters_non_dlm_role_groups(self):
        """TC-INT-ResUsers-004 — chỉ giữ lại group thuộc category RBAC DLM,
        group ngoài (vd base.group_system) bị lọc bỏ, không leo thang quyền."""
        Users = self.Users.with_user(self.admin_user)
