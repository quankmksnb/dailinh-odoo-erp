...
class TestPurchaseAccess(DlPurchaseCase):
    ...
    def test_thu_kho_khong_doc_duoc_gia_tren_lo(self):
        """🔴 MH-15 — thủ kho mở màn Lô và Tồn kho hằng ngày.

        Đỏ = giá mua rò ra cho vai trò không được biết, mà không ai phát hiện vì
        cột vẫn hiện bình thường với người có quyền.
        """
    ...
    def test_thu_kho_khong_mo_duoc_don_mua(self):
        """Kiểm soát chéo: người nhận hàng không phải người đặt hàng."""
        order = self._mk_po([(self.thep, 30.0, 200000.0)])
    ...
    def test_thu_kho_van_thay_so_don_mua_tren_phieu_nhan(self):
        """Nhưng vẫn phải BIẾT phiếu nhận này thuộc đơn mua nào.

        Đỏ = thủ kho cầm phiếu không đối chiếu được với giấy tờ NCC mang tới.
        Field related dùng sudo nên đọc được SỐ mà không đọc được giá.
        """
        order = self._mk_po([(self.thep, 30.0, 200000.0)])
    ...
    def test_mua_hang_khong_xac_nhan_duoc_phieu_kiem(self):
        """Đối xứng: người mua không tự nghiệm thu hàng mình mua."""
        order = self._mk_po([(self.thep, 30.0, 200000.0)])
        order.action_dlm_confirm()
        picking = order.dlm_picking_ids

        # ⚠️ `assertRaises` của Odoo KHÔNG nhận tuple (nó gọi issubclass trên
