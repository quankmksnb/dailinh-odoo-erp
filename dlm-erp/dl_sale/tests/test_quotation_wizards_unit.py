...
class TestQuotationWizards(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.customer = cls.env["res.partner"].create({
            ...
            "partner_type": "individual", "mobile": "0900001010",
        })
    ...
    def test_reject_wizard_other_reason_without_note_raises(self):
        """TC-INT-TestUntestedWizards-003: báo giá đang sent, wizard từ chối chọn reason
        "other" nhưng note rỗng thì action_confirm báo lỗi UserError.
        """
    ...
    def test_revision_wizard_empty_note_raises(self):
        """TC-INT-TestUntestedWizards-004: wizard yêu cầu chỉnh sửa với note chỉ toàn
        khoảng trắng thì action_confirm báo lỗi UserError.
        """
        quotation = self._quotation(state="sent")
    ...
    def test_revision_wizard_happy_delegates_to_apply_revision_request(self):
        """TC-INT-TestUntestedWizards-005: wizard yêu cầu chỉnh sửa kỹ thuật với note hợp
        lệ thì action_confirm chuyển báo giá sang state revision_requested, lưu đúng
        loại và nội dung yêu cầu, đồng thời trả về action đóng cửa sổ
    ...
    def test_sale_order_reset_draft_wizard_empty_reason_raises(self):
        """TC-INT-TestUntestedWizards-006: đơn hàng đang confirmed, wizard chuyển về nháp
        với lý do chỉ toàn khoảng trắng thì action_confirm báo lỗi ValidationError.
        """
    ...
    def test_export_wizard_email_blocked_when_customer_has_no_email(self):
        """TC-INT-TestUntestedWizards-007: khách hàng của báo giá chưa có email thì wizard
        xuất báo giá gọi action_email báo lỗi UserError.
        """
        quotation = self._quotation(state="sent")
    ...
    def test_export_wizard_email_happy_builds_composer_action(self):
        """TC-INT-TestUntestedWizards-008: khách hàng đã có email, wizard xuất báo giá gọi
        action_email trả về action mở mail.compose.message với context đúng
        model/res_ids của báo giá, có đính kèm file và đúng partner_ids của khách hàng.
        """
        quotation = self._quotation(state="sent")
