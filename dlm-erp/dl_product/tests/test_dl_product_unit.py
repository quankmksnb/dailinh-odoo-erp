# -*- coding: utf-8 -*-
"""Unit test L1 (thuần, không ORM/DB) cho product.product extension
(dl_product) — PROD-02. Sheet nguồn: ProductProduct.

Chỉ test method thuần Python hoặc method chỉ cần self.env cho 1-2 lời gọi cụ
thể (đã fake qua _FakeEnv/_fields giả, cùng kỹ thuật _fields đã dùng ở
dl_sale/dl_config). create/write/_dlm_find_name_matches/_check_lifecycle_
manager (dùng self.search/self.env.user.has_group nhiều nhánh) là L2, không
test ở đây.
"""
import unittest
...
class TestCheckCategBranch(unittest.TestCase):
    ...
    def test_other_branch_category_exempt(self):
        """TC-UNIT-ProductProduct-007 — nhóm chưa phân nhánh (vd 'All' mặc định
        core) không bị chặn, coi như 'chưa phân nhóm'."""
        rec = _product(product_kind="manufactured", dl_categ_branch="finished",
...
class TestDlmNormalizeName(unittest.TestCase):
    def test_collapses_whitespace_and_lowercases(self):
        """TC-UNIT-ProductProduct-013"""
        rec = _product()
        self.assertEqual(ProductProduct._dlm_normalize_name(rec, "  Khung  máy   ABC "),
                          "khung máy abc")

    def test_preserves_vietnamese_diacritics(self):
        """TC-UNIT-ProductProduct-014 — 'Khung máy ABC' và 'Khung may ABC'
        (thiếu dấu) KHÔNG được coi là trùng hệt theo chốt thiết kế."""
        rec = _product()
        self.assertNotEqual(
...
class FakeSellerRS(list):
    """Mini stand-in cho recordset — sau khi filtered()[:1] (như code thật
    làm), Odoo cho phép đọc field trực tiếp trên recordset 0/1 bản ghi
    (applied.approval_state, applied.price...) — __getattr__ dưới đây mô
    phỏng đúng ngữ nghĩa đó."""
